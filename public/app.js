...
const state={projects:[],project:null,view:'overview',books:[],book:null,chapter:null,pairs:[],selectedPair:null,files:[],issues:[],severity:'',selectedIssue:null};
const titles={overview:'Project overview',books:'Books & content',import:'Import center',qa:'QA review',language:'Language & style',publish:'Publish & export'};
...
function showView(name){state.view=name;$$('.view').forEach(v=>v.classList.remove('active'));$(`#view-${name}`)?.classList.add('active');$$('.nav-item[data-view]').forEach(b=>b.classList.toggle('active',b.dataset.view===name));$('#pageTitle').textContent=titles[name]||'Study Bible Creator';if(state.project)$('#breadcrumbs').textContent=`${state.project.name} · ${state.project.source_language_name||''} → ${state.project.target_language_name||''}`;else $('#breadcrumbs').textContent='Study Bible Creator';if(name==='overview')loadOverview();if(name==='books')loadBooksView();if(name==='qa')loadIssues();if(name==='language')loadLanguage();if(name==='publish')loadPublish();}
...
async function loadOverview(){
  ...
  $('#qaBadge').textContent=high||open;$('#qaBadge').classList.toggle('hidden',!open);
...
$('#runQa').onclick=runQa;$$('.filter').forEach(b=>b.onclick=async()=>{$$('.filter').forEach(x=>x.classList.remove('active'));b.classList.add('active');state.severity=b.dataset.severity;await loadIssues()});$('#refreshVocab').onclick=loadLanguage;$$('#exportGrid [data-format]').forEach(b=>b.onclick=()=>downloadExport(b.dataset.format));
