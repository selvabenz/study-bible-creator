...
const d=fs.mkdtempSync(path.join(os.tmpdir(),'sbc-mat-smoke-'));
...
assert.equal(issues.filter(x=>x.category==='versification_difference').length,4);
const stats=store.projectStats(project.id);
const result={sourceVerses:en.p.parsedSummary.verses,targetVerses:ta.p.parsedSummary.verses,pairedScripture:scripturePairs.filter(x=>x.source&&x.target).length,targetOnlyScripture:scripturePairs.filter(x=>!x.source&&x.target).map(x=>`${x.bookCode} ${x.chapter}:${x.verse}`),corrections:correctionReport,legacyComparisons,mapDocs,dbItems:stats.totals.items,protected:stats.totals.protected,qa};
