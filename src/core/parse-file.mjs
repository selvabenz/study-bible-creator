...
import { parseDoc } from './doc-parser.mjs';

export function detectFormat(filename, requested='auto') {
  if (requested && requested!=='auto') return requested.toLowerCase();
  const ext=path.extname(filename).toLowerCase();
  if (ext==='.usfm') return 'usfm'; if (ext==='.sfm') return 'sfm'; if (ext==='.csv') return 'csv'; if (ext==='.tsv') return 'tsv'; if (ext==='.json') return 'json'; if (ext==='.docx') return 'docx'; if (ext==='.doc') return 'doc';
...
export function parseFile(filePath, requested='auto') {
  ...
  if (format==='doc') return parseDoc(filePath);
